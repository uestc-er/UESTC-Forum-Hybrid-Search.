# 基于混合检索的校园论坛垂直搜索引擎 (MVP版)

一个单机版、轻量级的垂直领域搜索引擎，针对校园论坛特定版块（约1000条帖子）实现"语义 + 关键词"混合检索。

**告别"搜不到"，拒绝"搜不准"。一个懂语义、更懂校园生活的 AI 搜索引擎。**

## 项目概述

本项目旨在构建一个基于Python的本地混合搜索引擎，核心功能包括：
- **混合检索 (Hybrid Search)**：结合语义检索和关键词检索
- **倒数排名融合 (RRF)**：使用Reciprocal Rank Fusion算法合并不同检索方法的结果
- **轻量级单机部署**：无需GPU，适合普通笔记本电脑运行
- **完整的工作流程**：从数据爬取、索引构建到搜索服务的完整解决方案
